...
def login_page():
    ...
    # Botão de login
    if st.button("Login"):
        if username == "Bracell" and password == "BraC3ll!2023":
...
if not st.session_state.is_authenticated:
    ...
    path_adub = "Adubação_de_Cobertura_-_Silvicultura.xlsx"

    path_cad = 'Cadastro Florestal.xlsx'

    path_xy = "lat_long.xlsx"
    # ***Fim Paths***
    ...
    df_mapa['status'].replace(
        {'Conforme': 'Compliance', 'Não Conforme': 'Non - Compliance'}, inplace=True)

    cor_map = {'Compliance': '#A4D061',
               'Non - Compliance': '#C00000'}
